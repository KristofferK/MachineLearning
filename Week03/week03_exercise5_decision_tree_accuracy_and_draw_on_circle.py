...
X, y = make_circles(n_samples=sample_size, noise=0.1)
...
tree_clf = DecisionTreeClassifier(max_depth=2) # Circle is complicated, needs a big depth.
tree_clf.fit(X_training_set, y_training_set)
 
def draw_graph():
    cmap_bold = [ListedColormap(['#FFF000', '#00FFFF'])]
    pyplot.scatter(X[:, 0], X[:, 1], c=y, edgecolor='black', cmap=cmap_bold[0], s=20)
...
def draw_decision_tree():
    ...
    export_graphviz(tree_clf,
                    ...
                    rounded=True,
                    filled=True)
    filename = "tree_week03_exercise5.png"
...
draw_graph()
print(get_decision_tree_accuracy(False))
draw_decision_tree()